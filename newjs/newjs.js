...
function tablica14() {
    ...
    console.log(arab);
    document.getElementById("wynik14").innerHTML = arab;
}
//zad9
var janek=[];
function tablica15(){
    var dotab=parseFloat(document.getElementById("dodtab").value);
    janek.push(dotab);
    console.log(janek);
    document.getElementById("wynik15").innerHTML = janek;

}
function tablica16(){
    var sum = 0; // nie zapomnieć o wyzerowaniu!
for (var i = 0; i < janek.length; ++i) {
sum += janek[i];
}
console.log("suma: " + sum);
document.getElementById("wynik16").innerHTML = sum;
}
function tablica17() {
    var arnold = janek;
    var zmiana;
    do {
        zmiana = false;
        for (var i = 0; i < arnold.length - 1; ++i) {
            if (arnold[i] > arnold[i + 1]) {
                var tmp = arnold[i];
                arnold[i] = arnold[i + 1];
                arnold[i + 1] = tmp;
                zmiana = true;
    ...
    console.log(arnold);
    document.getElementById("wynik17").innerHTML = arnold;
}